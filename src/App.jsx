...
function SitePage({ site, go, health, setHealth }) {
  ...
  const KANKAN_TABS = [
    ["", "精选"], ["new", "最新"], ["albums", "相册"], ["premium", "VIP"],
  ];
  const DSD_TABS = [
  ...
  const HM_TABS = [
    ["latest", "最新上市"], ["uploaded", "最新上传"], ["裏番", "裏番"], ["泡麵番", "泡麵番"],
    ["Motion Anime", "Motion Anime"], ["3DCG", "3DCG"], ["2.5D", "2.5D"], ["2D動畫", "2D动画"],
    ["AI生成", "AI生成"], ["MMD", "MMD"], ["Cosplay", "Cosplay"],
  ];
  const HOJ_TABS = [
    ["", "最新"], ["cat:8", "亂倫"], ["cat:2", "強姦凌辱"], ["cat:12", "內射受孕"], ["cat:5", "多P群交"],
    ["cat:9", "巨乳美乳"], ["cat:7", "出軌"], ["cat:6", "角色劇情"], ["cat:1", "絲襪美腿"],
    ["cat:10", "潮吹放尿"], ["cat:11", "走後門"], ["cat:4", "制服誘惑"], ["cat:3", "主奴調教"],
  ];
  const MADOU_TABS = [
      ...
      const detail = provider.id === "eporner"
        ? await (async () => {
          ...
          if (streams?.streams) {
            ...
            // The official embed is the last-resort playback path when a
            // relay is unavailable or blocked by the current browser.
            card.media_kind = "embed";
          }
          return card;
        })()
        : provider.id === "kanxo"
  ...
  // The reference sites use single-page client/recommendation search for
  // these two entries.  Keep live upstream data, but do not expose a fake
  // search pager whose second request is not part of the reference UX.
  const singlePageSearch = Boolean(submitted && (provider?.id === "kan98" || provider?.id === "miss"));
  ...
  return <div className={`site-page accent-${site.accent} mode-${site.mode}`}>
    ...
    {provider?.id === "miss" && <div className="qiying-tabs">{MISS_TABS.map(([key, label]) => <button key={key} className={category === key ? "is-active" : ""} onClick={() => { setCategory(key); setSubmitted(""); setQuery(""); }}>{label}</button>)}</div>}
    ...
    {provider?.id === "hm" && <div className="qiying-tabs">{HM_TABS.map(([key, label]) => <button key={key} className={(key || "latest") === (category || "latest") ? "is-active" : ""} onClick={() => { setCategory(key); }}>{label}</button>)}</div>}
    {provider?.id === "hoj" && <div className="qiying-tabs">{HOJ_TABS.map(([key, label]) => <button key={key} className={(key || "home") === (category || "home") ? "is-active" : ""} onClick={() => { setCategory(key); }}>{label}</button>)}<button className={category === "cat" ? "is-active" : ""} onClick={() => { setCategory("cat"); }}>分类</button></div>}
    <section className="content-section"><div className="content-heading"><div><small>{submitted ? "SEARCH RESULT" : provider?.id === "rou" ? (category === "cat" ? "CATEGORY INDEX" : category === "tag" ? "TAG INDEX" : category.startsWith("tag:") ? "TAG WORKS" : "LATEST UPDATE") : category === "artists" ? "ARTIST INDEX" : category.startsWith("artist:") ? "ARTIST WORKS" : category === "videos" ? "ALL WORKS" : provider?.id === "js9" ? (category ? "CATEGORY BROWSE" : "HOME FEED") : provider?.id === "sf" ? (category ? "CATEGORY BROWSE" : "HOME FEED") : provider?.id === "jav" ? "JAV BROWSE" : provider?.id === "avjb" ? (category === "cat" ? "CATEGORY INDEX" : category.startsWith("cat:") ? "CATEGORY BROWSE" : category === "albums" ? "ALBUM GALLERY" : category === "premium" ? "VIP PREMIUM" : category === "new" ? "LATEST UPDATE" : "FEATURED") : provider?.id === "dsd" ? (category === "cat" ? "CATEGORY INDEX" : category.startsWith("cat:") ? "CATEGORY BROWSE" : category ? "CATEGORY BROWSE" : "LATEST UPDATE") : provider?.id === "hoj" ? (category === "cat" ? "CATEGORY INDEX" : category.startsWith("cat:") ? "CATEGORY BROWSE" : "LATEST UPDATE") : "LATEST UPDATE"}</small><h2>{submitted ? `“${submitted}”` : provider?.id === "rou" ? (category === "cat" ? "分类" : category === "tag" ? "标签" : category.startsWith("tag:") ? category.slice("tag:".length) : "首页") : category === "artists" ? "全部博主" : category.startsWith("artist:") ? category.slice("artist:".length) : category === "videos" ? "全部作品" : provider?.id === "js9" ? (category ? (JS9_TABS.find(([k]) => k === category)?.[1] || category) : "首页") : provider?.id === "sf" ? (category ? (SF_TABS.find(([k]) => k === category)?.[1] || category) : "首页") : provider?.id === "jav" ? (JAV_TABS.find(([k]) => k === category)?.[1] || "推荐") : provider?.id === "avjb" ? (category === "cat" ? "全部分类" : category.startsWith("cat:") ? category.slice(4) : (KANKAN_TABS.find(([k]) => k === category)?.[1] || "精选")) : provider?.id === "dsd" ? (category === "cat" ? "全部分类" : category.startsWith("cat:") ? (DSD_TABS.find(([k]) => k === category.slice(4))?.[1] || category.slice(4)) : (DSD_TABS.find(([k]) => k === category)?.[1] || "独家精选")) : provider?.id === "hoj" ? (category === "cat" ? "全部分类" : category.startsWith("cat:") ? (HOJ_TABS.find(([k]) => k === category)?.[1] || category.slice(4)) : (HOJ_TABS.find(([k]) => k === category)?.[1] || "最新")) : category ? (provider?.id === "madou" ? (MADOU_TABS.find(([k]) => k === category)?.[1] || category) : provider?.id === "hxc" ? (HXC_TABS.find(([k]) => k === category)?.[1] || "推荐") : "LATEST UPDATE") : site.mode === "live" ? "直播频道" : site.mode === "comic" ? "最新图册" : site.mode === "audio" ? "最新音声" : "最新内容"}</h2></div><span>{category === "artists" ? `${items.length} 位` : singlePageSearch ? `${items.length} 条` : `PAGE ${page}`}</span></div>
      ...
      {!loading && !error && provider?.id === "avjb" && category === "cat" && <div className="rou-tag-grid">{items.map((cat) => <button key={cat.vod_id} className="rou-tag-card" onClick={() => setCategory(`cat:${cat.vod_url.match(/categories\/([^/]+)\//)?.[1] || ""}`)}><strong>{cat.vod_name}</strong><small>{cat.vod_remarks}</small></button>)}</div>}
      {!loading && !error && provider?.id === "dsd" && category === "cat" && <div className="rou-tag-grid">{items.map((cat) => <button key={cat.vod_id} className="rou-tag-card" onClick={() => setCategory(cat.vod_id)}><strong>{cat.vod_name}</strong><small>{cat.vod_remarks}</small></button>)}</div>}
      {!loading && !error && provider?.id === "hoj" && category === "cat" && <div className="rou-tag-grid">{items.map((cat) => <button key={cat.vod_id} className="rou-tag-card" onClick={() => setCategory(cat.vod_id)}><strong>{cat.vod_name}</strong><small>{cat.vod_remarks}</small></button>)}</div>}
      {!loading && !error && !specialRou && !singlePageSearch && <div className="pager"><button disabled={page === 1} onClick={() => setPage((x) => Math.max(1, x - 1))}>上一页</button><span>{page}</span><button onClick={() => setPage((x) => x + 1)}>下一页</button></div>}
    </section>
...
function DetailModal({ item, mode, provider, onClose }) {
  ...
  useEffect(() => {
    ...
    const isDirectMedia = active.type === "video/mp4" || /\.(mp4|webm|ogg|m4v|mp3|m4a|aac)(?:$|[?&])/i.test(active.url);
    const isHls = active.type === "application/x-mpegURL" || /\.m3u8(?:$|\?)/i.test(active.url) || (!isDirectMedia && Hls.isSupported());
