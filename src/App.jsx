...
function SitePage({ site, go, health, setHealth }) {
  ...
  return <div className={`site-page accent-${site.accent} mode-${site.mode}`}>
    ...
    <section className="content-section"><div className="content-heading"><div><small>{submitted ? "SEARCH RESULT" : provider?.id === "rou" ? (category === "cat" ? "CATEGORY INDEX" : category === "tag" ? "TAG INDEX" : category.startsWith("tag:") ? "TAG WORKS" : "LATEST UPDATE") : category === "artists" ? "ARTIST INDEX" : category.startsWith("artist:") ? "ARTIST WORKS" : category === "videos" ? "ALL WORKS" : provider?.id === "js9" ? (category ? "CATEGORY BROWSE" : "HOME FEED") : provider?.id === "avjb" ? (category === "cat" ? "CATEGORY INDEX" : category.startsWith("cat:") ? "CATEGORY BROWSE" : category === "albums" ? "ALBUM GALLERY" : category === "premium" ? "VIP PREMIUM" : category === "new" ? "LATEST UPDATE" : "FEATURED") : "LATEST UPDATE"}</small><h2>{submitted ? `“${submitted}”` : provider?.id === "rou" ? (category === "cat" ? "分类" : category === "tag" ? "标签" : category.startsWith("tag:") ? category.slice("tag:".length) : "首页") : category === "artists" ? "全部博主" : category.startsWith("artist:") ? category.slice("artist:".length) : category === "videos" ? "全部作品" : provider?.id === "js9" ? (category ? (JS9_TABS.find(([k]) => k === category)?.[1] || category) : "首页") : provider?.id === "avjb" ? (category === "cat" ? "全部分类" : category.startsWith("cat:") ? category.slice(4) : (KANKAN_TABS.find(([k]) => k === category)?.[1] || "精选")) : category ? (provider?.id === "madou" ? (MADOU_TABS.find(([k]) => k === category)?.[1] || category) : "LATEST UPDATE") : site.mode === "live" ? "直播频道" : site.mode === "comic" ? "最新图册" : site.mode === "audio" ? "最新音声" : "最新内容"}</h2></div><span>{category === "artists" ? `${items.length} 位` : `PAGE ${page}`}</span></div>
