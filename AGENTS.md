...
- `qiying` / 栖影 was implemented on 2026-08-14: real upstream is 91吃瓜网 (Typecho, `agency.nsguiiwz.cc` + `being`/`act` mirror lines from its 防失联 page; `91vip2x` / `91chigua` domains are hijacked/retired). Directory/search/detail gallery use the one-time exported media-data mirror in `public/qiying/*.gz` (23368 posts, refreshed by re-running `scripts/prepare-qiying-data.mjs` on a fresh export); video URLs are fetched live from the main site's DPlayer `data-config` (server-signed `auth_key` m3u8 on `op.vkjyoi.cn`, ts/key on `bgqpnx.cn`, CORS `*`). No runtime dependency on cfnav.me. The 91吃瓜 videos cannot be played without the main-site signature; do not attempt to reuse cfnav.me `/api/hls`.
