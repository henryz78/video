...
| 下一项 | **下一阶段（2026-08-16）：浏览器全站对比验收**——由可打开真实浏览器的独立 agent 把全部已接入站点与参考站逐站对比（布局/数据/播放链路是否一致），验收完成后提交推送。已接入并本地 headless 验收过的站点：`qiying`、`madou`、`miss`、`tx`、`rou`、`mr`、`jm`、`hj`、`91`、`ai`、`lg`、`pmv`、`fj`、`movie`、`ep`、`tna`、`tv`(adulttv 39 路主题流已验，41 路 oxax 品牌流待可出网环境)；`qms`、`hqw`、`mt`、`best`、`xf`、`sjs`、`book`、`one` 用户决定跳过（风控/登录墙/私有票据/CF 全站保护/磁力下载站/上游不明/目录锁在登录墙后）；`hxc`、`dj`、`swag`、`kankan`、`9s`、`dsd`、`xo`、`jav`、`bj` 待接入；ASMR 按用户决定暂时跳过；`mm` / 墨影集 内容已由用户独立图库站「栖光集」完成（同批数据），聚合入口待接入、未来做外链跳转（链接未就绪）；`hj` 的完整正片 m3u8 探索（Richy 油猴脚本线索：preview 路径去 `_preview` 后缀）按用户指示暂缓 |
...
| 4 | `hj` | 看海角 | 社区 / feed | hj | — | 专用已验收 | 真实上游 `www.haijiao.com`（官方站，Vue SPA 数据全走 `/api/*` 匿名 JSON，`isEncrypted:true` 时 data 为**三层 Base64**：`JSON.parse(atob(atob(atob(text))))`）。2026-08-16 全链路破解并接入：热门列表 `/api/topic/hot/topics?page=N`（20/页）、分区树 `/api/topic/nodes_by_ver/v2?ver=`（128 节点）、分区帖子 `/api/topic/node/topics?type=1&nodeId=X&page=N`、搜索 `/api/topic/searchV2?q=&page=&limit=20`（total 上限 10000）、详情 `/api/topic/{id}`（`attachments[]`：images/video 附件带 `remoteUrl`）。**图片解密**：`pic.hj*.top` 的 `.txt` 是自定义 base64（字母表 `ABCD*EFGHIJKLMNOPQRSTUVWX#YZabcdefghijklmnopqrstuvwxyz1234567890`，`*`=`+`、`#`=`/`），解码即 `data:image/jpeg;base64,...`；代理 `action=img` 返回真实 JPEG。**视频播放**：未购买帖子只有约 30 秒 preview（25 片 × 1.25s），m3u8 在 `ts10.hj*.top/hjstore/video/{date}/{hash}/{id}_i_preview.m3u8`（CORS `*`，AES-128 带 IV）；`.key` 是包装 key，需官方 wasm `jquery_key(key, r)` 变换（r = m3u8 同目录 `.jpg` 文本 atob，即固定字符串——为不把上游泄露的 DB 凭据写进代码，r 每次动态现抓）；wasm 以 base64 内嵌 runtime.js 模块级缓存。`action=media` 重写 m3u8：KEY 行 URI → `action=key` 代理、ts 行 → 绝对直连；`action=key` 代理内做 wasm 变换返回真 key。付费边界与参考站一致：`sale.amount` 金币购买后才给全片，匿名只有 preview。headless 全链路验收：20 卡、封面 8/8、详情 14 图、1080×1920 播放推进、分区/搜索正常，零 JS 错误。复用 QiyingPage/QiyingModal（`provider.id==="hj"`）。零 cfnav 依赖。 |
...
| `hj` | `www.haijiao.com/api`（三层 Base64 匿名 JSON）+ `pic.hj*.top`（自定义 base64 加密图）+ `ts10.hj*.top`（AES-128 HLS preview） | 列表/分区树/分区帖子/搜索/详情全走匿名 API；图片 `action=img` 自定义 base64 解码返回真 JPEG；播放 `action=media` 重写 m3u8（KEY 行 → `action=key` 代理、ts 行绝对直连）；`action=key` 用内嵌 wasm `jquery_key` 变换（r 从 m3u8 同目录 `.jpg` 现抓） | HLS.js（AES-128，key 走代理、ts 直连） | 无账号；wasm 实例模块级缓存；r 不落盘 | 未购买帖子只有 30 秒 preview（上游付费边界）；wasm 是 emscripten 单函数模块，轮换概率低；CDN `ts10.hjNNN.top` 数字后缀可能轮换 |
...
- 2026-08-16 用户提供海角油猴脚本 `C:\Users\z6798\Downloads\Richy.txt`（2026-07-23.3，m3u8提取+去广告+原位播放）并指示**先记录、暂不实施**：核心机制已记录到 SOURCE-RESEARCH.md（剪贴板劫持五重钩子拦截、m3u8 捕获评分体系 100=`/api/address/` 反推 / 90=预览反推完整源 / 70=DOM / 50=普通 / 10=疑似预览、ts 分片名 LCP 反推主 m3u8 `xxx_i0.ts→xxx_i.m3u8`、`xxx_i_preview.m3u8→xxx_i.m3u8` 候选探测、≤45s/≤50 片判试看、三层 atob 与本地一致）。启示：完整正片 m3u8 可能 = preview 路径去 `_preview` 后缀（同目录同 key），脚本运行于登录/购买环境，匿名是否可拉未验证——后续再探索。
