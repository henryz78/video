...
- 2026-08-17 **17 站最终本地 Chrome 验收（不以参考站判定，未改代码）**：逐站等待列表、打开首卡详情、真实点击播放器并等待 20 秒左右；`ai` 额外抽测两个条目。通过：`ai` 24 卡，两条分别 `readyState=4/currentTime=34.89s/duration=1086.8s/1280×720` 与 `readyState=4/currentTime=31.67s/duration=1270.52s/854×480`；`hj` 20 卡，`32.65s/991.92s/720×720`；`91` 23 卡，`28.06s/645.72s/854×1508`；`mr` 27 卡，`27.54s/145.833s/1280×720`；`qiying` 15 卡，`21.51s/24s/720×1280`；`tx` 12 卡，`36.13s/3326.33s/1920×1080`；`rou` 153 卡，`28.39s/946.98s/1280×720`；`pmv` 24 卡，`29.22s/245.93s/1920×1080`；`lg` 24 卡（首卡为图集，另测首个 VIDEO 条目），`32.88s/82.04s/608×1081`；`fj` 24 卡，`39.12s/936.44s/1280×720`；`ep` 24 卡（iframe 内播放器），`33.06s/578s/361×480`；`tna` 24 卡，`30.45s/6544.07s/853×480`；`movie` 20 卡，`34.30s/6899.94s/722×406`；`madou` 20 卡，`37.36s/1394.89s/1280×720`；`jm` 48 卡，详情首章阅读器懒加载滚动后 23/23 页加载。上述均未出现 manifestLoadError、Failed to fetch 或 CORS 播放失败。`tv` 首页 24 卡，MyCamTV 搜索 15 卡，主题流 `readyState=4/currentTime=71.71s/duration=84.18s/1280×720`；oxax `ОХ-АХ HD` 当次等待后 `readyState=0/currentTime=0/videoWidth=0`，duration 动态增长至 75.52s，按已知“需部署级验证”记录为待验证，不判整体失败。`miss` 0 卡、页面明确显示 `上游返回 502`，仍为上游 CF 挑战阻塞。Console 仅见非阻塞的 React 空 `src` 警告，以及 eporner 第三方统计脚本的 cookie/弃用/`ajax3.js` 错误。
