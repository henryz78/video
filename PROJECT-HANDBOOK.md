...
- 2026-08-16 **第三轮浏览器补测（仅 `ai`、`madou`）**：使用独立 Chrome 会话，等待列表加载后对两端同一条目执行真实播放器手势；本轮未改代码。
  - `ai`：两端共同条目「精神小伙沉迷熟女阿姨的性感风骚无法自拔」（id `17765`）。本地列表/详情正常，实际点击后等待约 16 秒采样 `{readyState:0,currentTime:0,duration:null,videoWidth:0}`，视频仍暂停，控制台为 `hls fatal: networkError manifestLoadError`；Chrome 直连 `www.madouai.xyz` 主站页面正常，但本地播放器请求的 `https://www.madouai.xyz/api/v1/m3u8/proxy?...` 被浏览器报 `ERR_BLOCKED_BY_CLIENT`，故保留为**本地可用待参考（当前浏览器客户端拦截媒体路径，不能判适配器代码阻塞）**。参考端同条目点击播放后 `{readyState:4,currentTime:16.950322,duration:997.699994,videoWidth:480}`、持续播放、无控制台错误。
