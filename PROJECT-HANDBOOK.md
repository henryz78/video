...
| 下一项 | **2026-08-16 浏览器首轮对比已完成，但尚未宣告全站通过**——按用户要求只比较目录/数量/共同条目/搜索/详情/播放或阅读，不把 UI 背景和线路命名差异当作失败。FJ、EP、TX、MR 已在两边用同一条内容完成播放推进；其余差异和当前阻塞见文末的浏览器验收记录。`pmv` 空列表已查明为验收误判（health check 并发抢占连接，10 秒内正常显示 24 卡，已加 1.5s 超时优化）；`miss` 为上游 CF 全站挑战阻塞（调查中，参考站 FastAPI 服务器代抓路线待另一 agent 用真实浏览器+登录态查明）；播放未推进项（hj/qiying/ai/tv）多为观察窗口/手势问题，由另一 agent 复测。已接入并本地 headless 验收过的站点：`qiying`、`madou`、`tx`、`rou`、`mr`、`jm`、`91`、`ai`、`lg`、`pmv`、`fj`、`movie`、`ep`、`tna`、`tv`(adulttv 39 路主题流已验，41 路 oxax 品牌流待可出网环境)；`qms`、`hqw`、`mt`、`best`、`xf`、`sjs`、`book`、`one` 用户决定跳过（风控/登录墙/私有票据/CF 全站保护/磁力下载站/上游不明/目录锁在登录墙后）；`miss` 2026-08-16 起因上游 CF 挑战阻塞（调查中）；`hxc`、`dj`、`swag`、`kankan`、`9s`、`dsd`、`xo`、`jav`、`bj` 待接入；ASMR 按用户决定暂时跳过；`mm` / 墨影集 内容已由用户独立图库站「栖光集」完成（同批数据），聚合入口待接入、未来做外链跳转（链接未就绪）。`hj` 完整正片已实现（2026-08-16，Richy 线索验证有效并落地：preview ts 分片名 LCP 反推完整 m3u8，匿名可拉、无需金币） |
...
- 2026-08-16 **第二轮浏览器对比验收（按 30 秒观察 + 真实播放手势）**：
  ...
  - **阻塞**：`tv`（参考 `MyCamTV MILF` 主题流 duration=153.131s 且推进；本地同频道等待 36s 仍 readyState=0/time=0，属于本地 AdultIPTV HLS 播放链阻塞）；`miss`（本地列表/详情持续 502，上游访问被挑战；参考端可用，见下方调查）。
