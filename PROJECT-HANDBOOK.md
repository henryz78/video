...
- **新入口导航级探测（2026-08-17，内置浏览器登录态；未改代码）**：`sf` 私房 TV 的列表/详情/播放接口为 `/api/meta`、`/api/videos`、`/api/videos/{id}`、`/api/play/{id}`，无会话均 401；登录态播放器可播放授权后生成的 `14.29.46.204/aboxVOD/.../manifest.mpd`，但未找到公开目录或稳定独立上游，保持 PENDING。`98` 98堂页脚明确源站 `https://dmn12.vip`；参考站加载 `/data/site.json`、`groups.json`、`categories.json`、`videos.json`，点播 `/api/play/{id}` 无会话 401；登录态播放器可拿到 `tyjs.ypxjft.cn` 签名 HLS，manifest/key/TS 匿名直连 200+CORS，但尚未确认 dmn12.vip 的公开实时目录/详情接口，保持 PENDING。详细证据与重开条件已写入 `SOURCE-RESEARCH.md`，不复用 cfnav 登录会话、不复制静态目录、不接同类替代站。
