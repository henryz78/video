...
| 下一项 | `qiying`、`madou`、`miss`、`tx`、`rou` 已接入并本地验收；`qiying` 于 2026-08-15 转为全实时抓取（删除 gz 快照）；`qms`、`hqw`、`mt`、`best` 用户决定跳过（风控/登录墙/私有票据/CF 全站保护）；`xf` 2026-08-15 复评：目录仅登录墙、Twitter 官方 API 对成人推文全部 Tombstone、timeline 端点已废弃，无实时独立目录，按快照被否定的约束待用户裁定（倾向跳过）；ASMR 按用户决定暂时跳过；看 TV 的 41 路 oxax 品牌直播已实现、待可出网部署环境逐路验收 |
