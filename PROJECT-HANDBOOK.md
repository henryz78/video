...
| 下一项 | **下一阶段（2026-08-16）：浏览器全站对比验收**——由可打开真实浏览器的独立 agent 把全部已接入站点与参考站逐站对比（布局/数据/播放链路是否一致），验收完成后提交推送。已接入并本地 headless 验收过的站点：`qiying`、`madou`、`miss`、`tx`、`rou`、`mr`、`jm`、`91`、`ai`、`lg`、`pmv`、`fj`、`movie`、`ep`、`tna`、`tv`(adulttv 39 路主题流已验，41 路 oxax 品牌流待可出网环境)；`qms`、`hqw`、`mt`、`best`、`xf`、`sjs`、`book`、`one` 用户决定跳过（风控/登录墙/私有票据/CF 全站保护/磁力下载站/上游不明/目录锁在登录墙后）；`hxc`、`dj`、`swag`、`kankan`、`9s`、`dsd`、`xo`、`jav`、`bj` 待接入；ASMR 按用户决定暂时跳过；`mm` / 墨影集 内容已由用户独立图库站「栖光集」完成（同批数据），聚合入口待接入、未来做外链跳转（链接未就绪）。`hj` 完整正片已实现（2026-08-16，Richy 线索验证有效并落地：preview ts 分片名 LCP 反推完整 m3u8，匿名可拉、无需金币） |
