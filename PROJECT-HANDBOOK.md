...
| 下一项 | `qiying`、`madou`、`miss`、`tx`、`rou`、`mr` 已接入并本地验收；`qiying` 于 2026-08-15 转为全实时抓取（删除 gz 快照），`mr` 于 2026-08-15 复用 qiying 解析接入 mrds.com 实时上游；`qms`、`hqw`、`mt`、`best`、`xf`、`sjs` 用户决定跳过（风控/登录墙/私有票据/CF 全站保护/磁力下载站）；ASMR 按用户决定暂时跳过；看 TV 的 41 路 oxax 品牌直播已实现、待可出网部署环境逐路验收；`book` / 有声读物 为下一个待接入（音声模式） |
