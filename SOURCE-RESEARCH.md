...
- **实时目录传输层**：通用目录 relay（现为 `Vercel https://hy-relay.vercel.app/api?action=hm&path=` 主 → `r.jina.ai/http://hanime1.com` 兜底；原 Railway 主链路因用户更换域名暂时下线，待新域名回填后恢复为首选，`30s` 超时覆盖冷启动，保留原始 HTML、`CORS *`、不落盘、不复制 `hm` 静态 JSON）；实现 `hmPage`/`hmCards`/`hmPageCount`/`hmSearchPath`。Pages 直连 `r.jina.ai` 曾 `429`，Vercel 直抓 `hanime1.com` 亦 `403`，Railway 经住宅类出口可 `200` 含 `video-item-container`/`skip-page-form`。
