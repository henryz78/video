...
- **实时目录传输层**：通用三层 relay（`Railway https://relay-production-3a90.up.railway.app/api?action=hm&path=` 主 → `Vercel https://relay-lake-eight.vercel.app/api?action=hm&path=` 备 → `r.jina.ai/http://hanime1.com` 兜底，`30s` 超时覆盖 Railway 冷启动，保留原始 HTML、`CORS *`、不落盘、不复制 `hm` 静态 JSON）；实现 `hmPage`/`hmCards`/`hmPageCount`/`hmSearchPath`。Pages 直连 `r.jina.ai` 曾 `429`，Vercel 直抓 `hanime1.com` 亦 `403`，Railway 经住宅类出口可 `200` 含 `video-item-container`/`skip-page-form`。
